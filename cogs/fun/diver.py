...
class Diversao(commands.Cog):
    ...
    async def info(self, inter:disnake.ApplicationCommandInteraction, membro:disnake.Member):
        ...
        if len(data['dia']) == 1 and len(data['mes']) == 1:
            info.add_field(name='🔧 Criação:',value=f'0{data["dia"]}/0{data["mes"]}/{data["ano"]}',inline=True)
        else:
            info.add_field(name='🔧 Criação:',value=f'{data["dia"]}/{data["mes"]}/{data["ano"]}',inline=True)

        info.add_field(name='👤 User:', value=f'{membro.mention}', inline=True)
        ...
        async def press(inter:disnake.MessageInteraction):
            avatar = disnake.Embed(title='Avatar!', description=f'Avatar de {membro.name}', colour=disnake.Colour.from_rgb(128, 0, 255))
